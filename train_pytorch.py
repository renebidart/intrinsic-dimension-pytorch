...
import model
# from model import *
# from model.normalvae import *
from model.metric import vae_loss
...
scheduler = lr_scheduler.StepLR(optimizer, step_size=int(args.epochs/3), gamma=0.2)
...
class RepresentationExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.model.reparameterize(mu_logvar, deterministic=True)
        return x

# class RepresentationExtractor(nn.Module):
#     """Waste"""
#     def __init__(self, get_latent):
#         super(RepresentationExtractor, self).__init__()
